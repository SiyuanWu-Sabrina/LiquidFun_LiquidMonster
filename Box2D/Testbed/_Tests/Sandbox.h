...
class SpecialParticleTracker : public b2DestructionListener {
  public:
    ...
    SpecialParticleTracker()
        : m_world(NULL), m_particleSystem(NULL), m_colorOscillationTime(0.0f),
    ...
    void Add(const int32 *const particleIndices,
             const int32        numberOfParticles) {
        b2Assert(m_particleSystem);
        for(int32 i = 0; i < numberOfParticles &&
            m_particles.size() < SandboxParams::k_numberOfSpecialParticles;
            ++i) {
            ...
            m_particles.insert(
                m_particleSystem->GetParticleHandleFromIndex(particleIndex));
        }
    }
    ...
    void Step(float32 dt) {
        ...
        m_colorOscillationTime =
            fmodf(m_colorOscillationTime + dt, m_colorOscillationPeriod);
        const float32 colorCoeff = 2.0f *
            fabsf((m_colorOscillationTime / m_colorOscillationPeriod) - 0.5f);
        ...
        for(std::set<const b2ParticleHandle *>::const_iterator it =
            ...
            m_particleSystem->GetColorBuffer()[(*it)->GetIndex()] = color;
        }
    }
...
class Sandbox : public Test {
  public:
    Sandbox() {
        ...
        m_world->SetGravity(b2Vec2(10.0, 10.0));

        // Create physical box, no top
        {
            {
                b2PolygonShape shape;
                const b2Vec2   vertices[4] = {b2Vec2(-40, -10), b2Vec2(40, -10),
                                            b2Vec2(40, 0), b2Vec2(-40, 0)};
        ...
        m_killfieldShape.SetAsBox(k_playfieldRightEdge - k_playfieldLeftEdge,
                                  1);
    ...
    void SetupMaze() {
        ...
        for(int i = 0; i < k_tileWidth; i++) {
            for(int j = 0; j < k_tileHeight; j++) {
                ...
                b2Vec2 center = b2Vec2(k_playfieldLeftEdge +
                                           k_tileRadius * 2 * i + k_tileRadius,
                                       k_playfieldBottomEdge -
                                           k_tileRadius * 2 * j + k_tileRadius);
    ...
    virtual void Step(Settings *settings) {
        ...
        m_debugDraw.DrawString(5, m_textLine,
                               "Keys: (a) zero out (water), (q) powder");
        m_textLine += DRAW_STRING_NEW_LINE;
        m_debugDraw.DrawString(5, m_textLine, "      (t) tensile, (v) viscous");
        m_textLine += DRAW_STRING_NEW_LINE;
    }
...
const ParticleParameter::Value Sandbox::k_paramValues[] = {
    ...
    {b2_tensileParticle | b2_powderParticle,
     ParticleParameter::k_DefaultOptions, "tensile powder"},
    {b2_viscousParticle | b2_powderParticle,
     ParticleParameter::k_DefaultOptions, "viscous powder"},
    ...
    {b2_viscousParticle | b2_tensileParticle,
     ParticleParameter::k_DefaultOptions, "tensile viscous water"}};

const ParticleParameter::Definition Sandbox::k_paramDef[] = {
    {Sandbox::k_paramValues, B2_ARRAY_SIZE(Sandbox::k_paramValues)},
};
const uint32 Sandbox::k_paramDefCount = B2_ARRAY_SIZE(Sandbox::k_paramDef);
